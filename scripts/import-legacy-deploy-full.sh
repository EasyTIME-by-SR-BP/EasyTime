#!/usr/bin/env bash
# Legacy-Import für Deploy **inkl. Urlaub/Termine** (volle Historie).
#
...
if [[ -z "$SOURCE" || ! -f "$SOURCE" ]]; then
  echo "Usage: bash scripts/import-legacy-deploy-full.sh /pfad/zur/prod.sql" >&2
  exit 1
fi
...
echo "==> 1/5 Legacy-Dump nach SQLite (inkl. Urlaub/Termine)"
python3 database/convert_legacy_deploy_full.py --source "$SOURCE" --output database/database.sqlite
...
echo "==> 4/5 SQLite → MariaDB migrieren (Profil: full)"
compose --profile migrate run --rm -e MIGRATE_PROFILE=full migrate

echo "==> 5/5 Finalisierung in MariaDB"
compose run --rm web php scripts/finalize-legacy-import.php

echo ""
echo "Import abgeschlossen (inkl. Urlaub/Termine)."
