...
$topbarControlClass = 'inline-flex h-10 items-center rounded-xl border border-lime-200 bg-white transition-colors';
...
$inboxBadgeHtml = $unreadCount > 0
    ...
    : '';
$inboxBtnActive = $activeTab === 'inbox';
$inboxAriaLabel = $unreadCount > 0
    ? I18n::get('nav.inbox') . ' (' . $inboxBadgeLabel . ')'
    : I18n::get('nav.inbox');
...
?>
<header class="easytime-topbar z-50 flex min-h-14 sm:min-h-[4.5rem] shrink-0 items-center justify-between gap-2 sm:gap-4 overflow-visible border-b border-lime-200/70 bg-white/95 px-3 py-1 sm:px-6 lg:px-8 lg:sticky lg:top-0 max-lg:pt-[env(safe-area-inset-top,0px)] shadow-sm backdrop-blur-lg">
    <a href="<?= $isAdmin ? '/?tab=operations' : '/?tab=calendar' ?>" class="flex min-w-0 items-center gap-2 sm:gap-3">
        <img src="/assets/icons/urlaubsplaner_icon.svg" alt="EasyTime" class="h-9 w-9 sm:h-10 sm:w-10 shrink-0 rounded-xl shadow-md shadow-lime-400/20">
        <div class="min-w-0">
            <span class="block truncate text-lg font-bold tracking-tight text-emerald-900 sm:text-2xl">Easy<span class="text-lime-600">Time</span></span>
    ...
    <div class="flex shrink-0 items-center gap-1.5 overflow-visible sm:gap-3">
        <div class="<?= $topbarControlClass ?> gap-1 px-2 sm:px-3 text-xs sm:text-sm">
        ...
        <div class="relative" x-data="{ open: false }" @keydown.escape.window="open = false">
            <button
                type="button"
                class="<?= $topbarControlClass ?> gap-2 rounded-full py-0 pl-1 pr-3 text-left hover:border-lime-300 hover:bg-lime-50"
                :class="open ? 'border-lime-400 ring-2 ring-lime-200' : ''"
                @click="open = !open"
                aria-haspopup="true"
                :aria-expanded="open.toString()"
            >
                <span class="et-avatar flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold uppercase shadow-inner"><?= $userInitial ?></span>
